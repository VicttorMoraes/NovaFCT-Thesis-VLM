...
Although variance is a convenient and widely used proxy for risk, it captures overall dispersion and may fail to distinguish between moderate variability and extreme downside outcomes. For this reason, the portfolio optimization literature proposes several alternative risk measures with different mathematical properties and computational implications. Linear deviation-based measures such as Mean Absolute Deviation (MAD) can approximate risk while preserving linear structure, often leading to MILP formulations that scale better than quadratic variance models \parencite{MansiniOgryczakSperanza2014}. Tail-focused measures such as Conditional Value-at-Risk (CVaR) explicitly target extreme losses and are widely adopted in scenario-based optimization because they can be formulated using linear constraints \parencite{Rockafellar2001CVaR}. More recently, non-linear coherent measures such as Entropic Value-at-Risk (EVaR) have been proposed to overcome limitations of VaR/CVaR monotonicity and to provide tractable convex formulations in sample-based settings \parencite{AhmadiJavid2012EVaR}. Collectively, these measures highlight that "risk" can be operationalized in multiple linear and non-linear ways, motivating the broader risk perspective adopted in this dissertation \parencite{PichlerSchiller2015RiskMeasures}.

TThe primary objective of this thesis is to develop and analyze a risk-aware optimization framework for urban fleet replacement decisions under uncertainty, using mean-variance as the baseline formulation and positioning it relative to alternative linear and non-linear risk measures. Specifically, this work aims to:
