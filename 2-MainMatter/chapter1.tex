...
Beyond mean-variance portfolio formulations, the fleet replacement problem has been addressed through a variety of Operations Research approaches. A common baseline is deterministic life-cycle or mixed-integer programming (MIP) modeling, where replacement timing and fleet mix are optimized under budget and operational constraints, often incorporating environmental costs or emissions policies. For instance, \textcite{FengFigliozzi2013} propose an integer programming framework to analyze the economic competitiveness of electric versus conventional commercial vehicles across utilization and price scenarios Similarly, \textcite{FigliozziBoudartFeng2011}, develops a vehicle replacement model that explicitly captures trade-offs between economic and environmental objectives, including taxes and incentives. In the public transport context, \textcite{Pelletier2019EBFTP} formulate the electric bus fleet transition problem, integrating replacement planning with charging infrastructure investment decisions and electrification targets.

More recent studies extend these replacement models by embedding uncertainty and policy mechanisms directly into the decision process. For example, \textcite{Xiao2023} develop a fleet replacement model under a cap-and-trade system while accounting for uncertainty in government subsidies and alternative business models (leasing). From an operationally perspective, \textcite{Malladi2020} formulate a two-stage stochastic program for fleet size and mix decisions under uncertain customer requests and temperature-dependent energy consumption, highlighting that fleet composition decisions can be materially affected by operational variability. Collectively, these approaches show that fleet replacement planning focus on deterministic, policy-constrained, and stochastic paradigms, however, they typically do not provide an explicit cost-risk efficient frontier under uncertainty as portfolio theory does.
